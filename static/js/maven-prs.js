(() => {
  ...
  function MavenPRs({ status }) {
    const [prs, setPrs] = (0, import_react.useState)(null);
    (0, import_react.useEffect)(() => {
      async function fetchPRs() {
        ...
        const relevantProjects = projects.filter((p) => relevantUuids.includes(p.uuid));
        if (relevantProjects.length === 0) {
          setPrs([]);
          return;
        }
        const matchingPatterns = relevantProjects.flatMap((p) => p.matchingRepos);
        const matchesRepo = (org, repo) => {
          return matchingPatterns.some((pattern) => {
            if (pattern.includes("*")) {
        ...
        const filtered = allPRs.filter((pr) => {
          ...
          const statusMatches = !status || status === "open" && pr.open || status === "merged" && pr.merged;
    ...
    if (!prs)
      return /* @__PURE__ */ import_react.default.createElement("div", null, "Keine Pull Requests gefunden");
  ...
  // maven-prs-wrapper.tsx
  var MavenPRsElement = class extends HTMLElement {
    connectedCallback() {
      const status = this.getAttribute("status");
      const root = (0, import_client.createRoot)(this);
      root.render(/* @__PURE__ */ import_react2.default.createElement(MavenPRs, { status }));
    }
  };
