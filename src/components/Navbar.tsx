...
export default function Navbar({ locale }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('scroll', updateIllustrationOffset, {
      passive: true,
    });
  ...
  useEffect(() => {
    ...
    fetch(`/api/post-locales/${encodedSlug}?locale=${locale}`)
      .then(async response => {
        ...
        const alternates = data.alternates.filter((alt: { locale: string }) =>
          allLocales.includes(alt.locale as SupportedLocale),
        );
  ...
  const getLocaleHref = (targetLocale: SupportedLocale): string => {
    if (postSlug && postLocaleAlternates?.slug === postSlug) {
      const alt = postLocaleAlternates.alternates.find(
        a => a.locale === targetLocale,
      );
      if (alt) {
  ...
  const renderLocaleSwitcher = (variant: 'desktop' | 'mobile') => {
    ...
    return (
      <div
        ...
        data-locale-switcher-group={variant}>
